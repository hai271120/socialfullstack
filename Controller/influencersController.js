...
const influencerCtrl = {
    getInfluencer: async(req, res) =>{
        try {
            ...
            const influencer = await features.query

            res.json({
                status: 'success',
                result: influencer.length,
    ...
    createInfluencer: async(req, res) =>{
        try {
            const {influencer_id, facebook,address,name,images,instagram,tiktok,category,followface,followinstagram,followtiktok} = req.body;
            ...
            const newInfluencer = new Influencer({
                influencer_id, name: name.toLowerCase(), tiktok, instagram,facebook, images,address,category,followface,followinstagram,followtiktok
            })
    ...
    updateInfluencer: async(req, res) =>{
        try {
            ...
            await Influencer.findOneAndUpdate({_id: req.params.id}, {
                name: name.toLowerCase(),  tiktok, instagram,facebook, images,address,category,followface,followinstagram,followtiktok
            })
